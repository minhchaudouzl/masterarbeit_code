...
# exponentiierte Weibullverteilung mit 3 Parametern

getexpweib <- function (p = c(0.025, 0.5, 0.975), q, show.output = TRUE, plot = TRUE, 
                       start,
                                                                                ...
                                                                                0.9), ...) 
{
  ...
  minimize <- function(theta) {
    summand <- suppressWarnings((1 - exp(-(q / theta[1])^(theta[2])))^(theta[3]) - p)
  ...
  if (is.error(try1) || fit$value >= tol) {
    ...
    if (is.error(try2) || fit$value >= tol) {
      ...
      Par <- fit$par
      names(Par) <- c("scale", "1.shape", "2.shape")
      if (show.output) 
        print(fit)
    }
  }
  ...
  if (prod(!is.na(Par)) & plot) {
    main1 <- paste("scale = ", signif(Par["scale"], 
                                     digits = 3))
    main2 <- paste("1.shape = ", signif(Par["1.shape"], 
                                     digits = 3)) 
    main3 <- paste("2.shape = ", signif(Par["2.shape"], 
                                    digits = 3))
    main <- paste("exp. Weibull (", main1, ", ", main2, ", ", main3, ")", sep = "")
    
    sub = paste("fit.weights = c(", paste(fit.weights.original, 
                                          collapse = ", "), ")", sep = "")
    Support.lim <- c(0, 60)
    Support <- seq(Support.lim[1], Support.lim[2], length = 200)
    Probability <- (1 - exp(-(Support / Par["scale"])^(Par["1.shape"])))^(Par["2.shape"])
