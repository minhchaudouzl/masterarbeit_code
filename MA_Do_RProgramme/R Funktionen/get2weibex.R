...
# Mischung aus exponentiierter Weibull und Exponentialverteilung

get2weibex <- function (p = c(0.025, 0.5, 0.975), q, show.output = TRUE, plot = TRUE, 
                       start,
                                                                                ...
                                                                                0.9), ...) 
{
  ...
  minimize <- function(theta) {
    summand <- suppressWarnings((exp(theta[4]) / ( 1 + exp(theta[4])) * 
                                   reliaR::pexpo.weibull(q = q,
                                                         alpha = theta[1],
                                                         theta = theta[2]) +
                                   (1 - exp(theta[4]) / ( 1 + exp(theta[4]))) * 
                                   stats::pexp(q = q, rate = theta[3])) - p)
  ...
  if (is.error(try1) || fit$value >= tol) {
    ...
    if (is.error(try2) || fit$value >= tol) {
      ...
      Par <- fit$par
      names(Par) <- c("alpha", "theta", "rate", "mix")
      if (show.output) 
        print(fit)
    }
  }
  ...
  if (prod(!is.na(Par)) & plot) {
    main1 <- paste("alpha=", signif(Par["alpha"], 
                                     digits = 3))
    main2 <- paste("theta=", signif(Par["theta"], 
                                     digits = 3)) 
    main3 <- paste("rate=", signif(Par["rate"], 
                                    digits = 3))
    main4 <- paste("mix=", signif(exp(Par["mix"]) / ( 1 + exp(Par["mix"])), 
                                   digits = 3))
    main <- paste("exp.Weibull&Exponetial(", main1, ", ", main2, ", ", main3, ", ", main4,
                  ")", sep = "")
    sub = paste("fit.weights = c(", paste(fit.weights.original, 
                                          collapse = ", "), ")", sep = "")
    Support.lim <- c(0.0001, 60)
    Support <- seq(Support.lim[1], Support.lim[2], length = 200)
    Probability <- (exp(Par["mix"]) / ( 1 + exp(Par["mix"])) * 
                      reliaR::pexpo.weibull(q = Support,
                                            alpha = Par["alpha"],
                                            theta = Par["theta"]) +
                      (1 - exp(Par["mix"]) / ( 1 + exp(Par["mix"]))) * stats::pexp(q = Support,
                                                                                   rate = Par["rate"]))
