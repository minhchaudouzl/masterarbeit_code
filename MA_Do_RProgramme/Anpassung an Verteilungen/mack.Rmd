...
mack_b <- read.csv2("Rekonstruktion/Mack/blau.csv", sep = ";", header = FALSE)
mack_r <- read.csv2("Rekonstruktion/Mack/rot.csv", sep = ";", header = FALSE)
```
...
mack_r$V2 <- 100 - mack_r$V2

mack_r <- mack_r[complete.cases(mack_r),]

mack_r <- mack_r[order(mack_r$V2), ]

# Werte außerhalb des Bereichs [0, inf] raus
mack_r <- mack_r[mack_r$V1 >= 0, ]
xr_mack <- sort(mack_r$V1)
yr_mack <- mack_r$V2



# In Mortalität umrechnen
mack_b$V2 <- 100 - mack_b$V2

mack_b <- mack_b[complete.cases(mack_b),]

mack_b <- mack_b[order(mack_b$V2), ]

# Werte außerhalb des Bereichs [0, inf] raus
mack_b <- mack_b[mack_b$V1 >= 0, ]
xb_mack <- sort(mack_b$V1)
yb_mack <- mack_b$V2
```
...
find_best_start_2parameter <- function(p, q, max_beta, max_eta, steps_beta,
  ...
  for (beta in seq(0, max_beta, by = steps_beta)) {
    for (eta in seq(0, max_eta, by = steps_eta)) {
      ...
      # Berechne den Fehler (mack_r_1$value) für die aktuellen Startparameter
...
# Datenanpassung an die Daten von Mack

## Weibullverteilung

```{r mackr getweibullpar, warning=FALSE, messages = FALSE, error=FALSE}
# Weibullverteilung
source("D:/MA_Do_RProgramme/R Funktionen/getweibullpar.R")

best_mack_r_1 <- find_best_start_2parameter(p = yr_mack/100, q = xr_mack, 
                                               max_beta = 10, max_eta = 10, 
                                               steps_beta = 1, steps_eta = 1, 
                                               fitting_function = getweibullpar)


mack_r_1 <- getweibullpar(
                        p = yr_mack/100,
                        q = xr_mack,
                        start = best_mack_r_1,
                        show.output = TRUE,
...
plot(xr_mack,
     pweibull(xr_mack,
              scale = mack_r_1["scale"], 
              shape = mack_r_1["shape"]), type = "l")
```

```{r mackb getweibullpar, warning=FALSE, messages = FALSE, error=FALSE}
best_mack_b_1 <- find_best_start_2parameter(p = yb_mack/100, q = xb_mack, 
                                               max_beta = 10, max_eta = 10, 
                                               steps_beta = 1, steps_eta = 1, 
                                               fitting_function = getweibullpar)

mack_b_1 <- getweibullpar(
                        p = yb_mack/100,
                        q = xb_mack,
                        start = best_mack_b_1,
                        show.output = TRUE,
...
plot(xb_mack,
     pweibull(xb_mack,
              scale = mack_b_1["scale"], 
              shape = mack_b_1["shape"]), type = "l")
...
```{r mackr getweibpar, warning=FALSE, messages = FALSE, error=FALSE}
# exponentiierte Weibullverteilung
source("D:/MA_Do_RProgramme/R Funktionen/getweibpar.R")

best_weibbull_xr_mack <- find_best_start_2parameter(p = yr_mack/100, 
                                                     q = xr_mack,
                                                     max_beta = 10, 
                                                     max_eta = 10,
                                                     steps_beta = 1, 
...
weibbull_xr_mack <- getweibpar(
                        p = yr_mack/100,
                        q = xr_mack,
                        start = best_weibbull_xr_mack,
                        show.output = TRUE,
                        plot = TRUE
                      )
weibbull_xr_mack
...
plot(xr_mack,
     pexpo.weibull(xr_mack,
              alpha = weibbull_xr_mack ["alpha"],
              theta = weibbull_xr_mack ["theta"]), type = "l")
```

```{r mack getweibpar, warning=FALSE, messages = FALSE, error=FALSE}
best_weibbull_xb_mack <- find_best_start_2parameter(p = yb_mack/100, 
                                                     q = xb_mack,
                                                     max_beta = 10, 
                                                     max_eta = 10,
                                                     steps_beta = 1, 
...
weibbull_xb_mack  <- getweibpar(
                        p = yb_mack/100,
                        q = xb_mack,
                        start = best_weibbull_xb_mack,
                        show.output = TRUE,
                        plot = TRUE
                      )
weibbull_xb_mack
...
plot(xb_mack,
     pexpo.weibull(xb_mack,
              alpha = weibbull_xb_mack ["alpha"],
              theta = weibbull_xb_mack ["theta"]), type = "l")
```


## Mischung aus Weibull- und Exponentialverteilung

```{r mackr getweibex, warning=FALSE, messages = FALSE, error=FALSE}
# Mischung aus Weibull- und Exponentialverteilung
source("D:/MA_Do_RProgramme/R Funktionen/getweibex.R")

best_weibex_xr_mack <- find_best_start_4parameter(p = yr_mack/100, 
                                                   q = xr_mack, 
                                                   max_shape = 1, 
                                                   max_scale = 100, 
                                                   max_rate = 0.7, 
...
weibex_xr_mack <- getweibex(
                        p = yr_mack/100,
                        q = xr_mack,
                        start = best_weibex_xr_mack, # c(0.5, 60, 0.4, 0.1),
                        show.output = TRUE,
                        plot = TRUE
                      )
weibex_xr_mack
...
plot(xr_mack,
     (exp(weibex_xr_mack["mix"]) / ( 1 + exp(weibex_xr_mack["mix"])) * 
        ...
        stats::pexp(q = xr_mack,
                    rate = weibex_xr_mack["rate"])),
     type = "l")
```


```{r mackb getweibex, warning=FALSE, messages = FALSE, error=FALSE}
best_weibex_xb_mack <- find_best_start_4parameter(p = yb_mack/100, 
                                                   q = xb_mack, 
                                                   max_shape = 1, 
                                                   max_scale = 100, 
                                                   max_rate = 0.7, 
...
weibex_xb_mack <- getweibex(
                        p = yb_mack/100,
                        q = xb_mack,
                        start = best_weibex_xb_mack, # c(0.5, 60, 0.7, 0.1),
                        show.output = TRUE,
                        plot = TRUE
                      )
weibex_xb_mack
...
plot(xb_mack,
     (exp(weibex_xb_mack["mix"]) / ( 1 + exp(weibex_xb_mack["mix"])) * 
        ...
        stats::pexp(q = xb_mack,
                    rate = weibex_xb_mack["rate"])),
     type = "l")
```

## Mischung von exponentiierter Weibull- und Exponentialverteilung

```{r mackr get2weibex, warning=FALSE, messages = FALSE, error=FALSE}
# Mischung von exponentiierter Weibull- und Exponentialverteilung
source("D:/MA_Do_RProgramme/R Funktionen/get2weibex.R")

best_weibex2_xr_mack <- find_best_start_4parameter(p = yr_mack/100, 
                                                    q = xr_mack,
                                                    max_shape = 1, 
                                                    max_scale = 100,
                                                    max_rate = 0.7, 
...
weibex2_xr_mack <- get2weibex(
                        p = yr_mack/100,
                        q = xr_mack,
                        start = best_weibex2_xr_mack,
                        show.output = TRUE,
                        plot = TRUE
                      )
weibex2_xr_mack
...
plot(xr_mack,
     (exp(weibex2_xr_mack["mix"]) / ( 1 + exp(weibex2_xr_mack["mix"])) *
        ...
        stats::pexp(q = xr_mack,
                    rate = weibex2_xr_mack["rate"])),
     type = "l")
```

```{r mackb get2weibex, warning=FALSE, messages = FALSE, error=FALSE}
best_weibex2_xb_mack <- find_best_start_4parameter(p = yb_mack/100, 
                                                    q = xb_mack,
                                                    max_shape = 1, 
                                                    max_scale = 100,
                                                    max_rate = 0.7, 
...
weibex2_xb_mack <- get2weibex(
                        p = yb_mack/100,
                        q = xb_mack,
                        start = best_weibex2_xb_mack,
                        show.output = TRUE,
                        plot = TRUE
                      )
weibex2_xb_mack
...
```{r mackr getexpweib, warning=FALSE, messages = FALSE, error=FALSE}
# exponentiierte Weibullverteilung ohne Lambda = 1
source("D:/MA_Do_RProgramme/R Funktionen/getexpweib.R")

best_expweib_xr_mack <- find_best_start_3parameter(p = yr_mack/100, 
                                                    q = xr_mack, 
                                                    max_shape1 = 10, 
                                                    max_shape2 = 10, 
                                                    max_scale = 10, 
...
expweib_xr_mack <- getexpweib(
                        p = yr_mack/100,
                        q = xr_mack,
                        start = best_expweib_xr_mack,
                        show.output = TRUE,
                        plot = TRUE
                      )
expweib_xr_mack
...
plot(xr_mack,
     (1 - exp(-(xr_mack / expweib_xr_mack["scale"])^
                      (expweib_xr_mack["1.shape"]))) ^ (expweib_xr_mack["2.shape"]),
     type = "l")
```

```{r mackb getexpweib, warning=FALSE, messages = FALSE, error=FALSE}
best_expweib_xb_mack <- find_best_start_3parameter(p = yb_mack/100, 
                                                    q = xb_mack, 
                                                    max_shape1 = 10, 
                                                    max_shape2 = 10, 
                                                    max_scale = 10, 
...
expweib_xb_mack <- getexpweib(
                        p = yb_mack/100,
                        q = xb_mack,
                        start = best_expweib_xb_mack,
                        show.output = TRUE,
                        plot = TRUE
                      )
expweib_xb_mack
...
plot(xb_mack,
     (1 - exp(-(xb_mack / expweib_xb_mack["scale"])^(expweib_xb_mack["1.shape"])))^(expweib_xb_mack["2.shape"]),
     type = "l")
```

## 3-Stufige Exponetialverteilung

```{r mackr getstuexp3, warning=FALSE, messages = FALSE, error=FALSE}
# 3-Stufige Exponetialverteilung
source("D:/MA_Do_RProgramme/R Funktionen/getstuexp3.R")

best_stuexp3_xr_mack <- find_best_start_3parameter(p = yr_mack/100, 
                                                    q = xr_mack, 
                                                    max_shape1 = 0.1, 
                                                    max_shape2 = 0.1, 
                                                    max_scale = 0.1, 
...
stuexp3_xr_mack <- getstuexp3(
                        p = yr_mack/100,
                        q = xr_mack,
                        start = best_stuexp3_xr_mack,
                        show.output = TRUE,
                        plot = TRUE,
                        wert1 = 2,
                        wert2 = 6
...
plot(xr_mack,
     ((xr_mack > 0 & xr_mack <= 2 ) * (1 - exp(-stuexp3_xr_mack[1] * xr_mack)) + 
              ...
              (1 - exp(-stuexp3_xr_mack[1] * 2))  + 
                                   (exp(-2 * stuexp3_xr_mack[2]) - 
                                            exp(-stuexp3_xr_mack[2] * xr_mack)) +
                                   (xr_mack > 6 & xr_mack <= 65 ) * 
              (1 - exp(-stuexp3_xr_mack[1] * 2)) + 
                                   (exp(-2 * stuexp3_xr_mack[2]) - exp(-6 * stuexp3_xr_mack[2])) + 
                                   (exp(-6 * stuexp3_xr_mack[3]) - 
                                            exp(-stuexp3_xr_mack[3] * xr_mack))),
     type = "l")
```

```{r mackb getstuexp3, warning=FALSE, messages = FALSE, error=FALSE}
best_stuexp3_xb_mack <- find_best_start_3parameter(p = yb_mack/100, 
                                                    q = xb_mack, 
                                                    max_shape1 = 0.1, 
                                                    max_shape2 = 0.1, 
                                                    max_scale = 0.1, 
...
stuexp3_xb_mack <- getstuexp3(
                        p = yb_mack/100,
                        q = xb_mack,
                        start = best_stuexp3_xb_mack,
                        show.output = TRUE,
                        plot = TRUE,
                        wert1 = 2,
                        wert2 = 6
     ...
     ((xb_mack > 0 & xb_mack <= 2 ) * (1 - exp(-stuexp3_xb_mack[1] * xb_mack)) + 
              ...
              (1 - exp(-stuexp3_xb_mack[1] * 2)) + 
                                   ...
                                   (exp(-6 * stuexp3_xb_mack[3]) - 
                                            exp(-stuexp3_xb_mack[3] * xb_mack))),
     type = "l")
```

## 2-Stufige Exponetialverteilung

```{r mackr getstuexp2, warning=FALSE, messages = FALSE, error=FALSE}
# 2-Stufige Exponetialverteilung
source("D:/MA_Do_RProgramme/R Funktionen/getstuexp2.R")

best_stuexp2_xr_mack <- find_best_start_2parameter(p = yr_mack/100, 
                                                    q = xr_mack, 
                                                    max_beta = 1, 
                                                    max_eta = 0.5, 
                                                    steps_beta = 0.1, 
...
stuexp2_xr_mack <- getstuexp2(
                        p = yr_mack/100,
                        q = xr_mack,
                        start = best_stuexp2_xr_mack,
                        show.output = TRUE,
                        plot = TRUE,
                        wert1 = 2
                      )
...
plot(xr_mack,
     ((xr_mack > 0 & xr_mack <= 2 ) * (1 - exp(-stuexp2_xr_mack[1] * xr_mack)) + 
                                   (xr_mack > 2 ) *  (1 - exp(-stuexp2_xr_mack[1] * 2))  + 
                                   (exp(-2 * stuexp2_xr_mack[2]) - 
                                            exp(-stuexp2_xr_mack[2] * xr_mack))),
     type = "l")
```

```{r mackb getstuexp2, warning=FALSE, messages = FALSE, error=FALSE}
best_stuexp2_xb_mack <- find_best_start_2parameter(p = yb_mack/100, 
                                                    q = xb_mack, 
                                                    max_beta = 1, 
                                                    max_eta = 0.5, 
                                                    steps_beta = 0.1, 
...
stuexp2_xb_mack <- getstuexp2(
                        p = yb_mack/100,
                        q = xb_mack,
                        start = best_stuexp2_xb_mack,
                        show.output = TRUE,
                        plot = TRUE,
                        wert1 = 2
                      )
...
plot(xb_mack,
     ((xb_mack > 0 & xb_mack <= 2 ) * (1 - exp(-stuexp2_xb_mack[1] * xb_mack)) + 
                                   (xb_mack > 2 ) *  (1 - exp(-stuexp2_xb_mack[1] * 2))  + 
                                   (exp(-2 * stuexp2_xb_mack[2]) - 
                                            exp(-stuexp2_xb_mack[2] * xb_mack))),
     type = "l")
```

## Mischung aus 2 Exponentialverteilungen

```{r mackr getexex, warning=FALSE, messages = FALSE, error=FALSE}
# Mischung aus 2 Exponentialverteilungen
source("D:/MA_Do_RProgramme/R Funktionen/getexex.R")

best_exex_xr_mack <- find_best_start_3parameter(p = yr_mack/100, 
                                                 q = xr_mack, 
                                                 max_shape1 = 0.7, 
                                                 max_shape2 = 0.7, 
                                                 max_scale = 1, 
...
exex_xr_mack <- getexex(
                        p = yr_mack/100,
                        q = xr_mack,
                        start = best_exex_xr_mack,
                        show.output = TRUE,
                        plot = TRUE
                      )
...
plot(xr_mack,
     (exp(exex_xr_mack["mix"]) / ( 1 + exp(exex_xr_mack["mix"])) *  stats::pexp(q = xr_mack,
                                                                            rate = exex_xr_mack["rate1"]) +
                      (1 - exp(exex_xr_mack["mix"]) / ( 1 + exp(exex_xr_mack["mix"]))) * 
              stats::pexp(q = xr_mack,
                          rate = exex_xr_mack["rate2"])),
     type = "l")
```

```{r mackb getexex, warning=FALSE, messages = FALSE, error=FALSE}
best_exex_xb_mack <- find_best_start_3parameter(p = yb_mack/100, 
                                                 q = xb_mack, 
                                                 max_shape1 = 0.7, 
                                                 max_shape2 = 0.7, 
                                                 max_scale = 1, 
...
exex_xb_mack <- getexex(
                        p = yb_mack/100,
                        q = xb_mack,
                        start = best_exex_xb_mack,
                        show.output = TRUE,
                        plot = TRUE
                      )
...
plot(xb_mack,
     (exp(exex_xb_mack["mix"]) / ( 1 + exp(exex_xb_mack["mix"])) *  stats::pexp(q = xb_mack,
                                                                            rate = exex_xb_mack["rate1"]) +
                      (1 - exp(exex_xb_mack["mix"]) / ( 1 + exp(exex_xb_mack["mix"]))) * 
              stats::pexp(q = xb_mack,
                          rate = exex_xb_mack["rate2"])),
     type = "l")
```
...
```{r mack_result, warning=FALSE, messages = FALSE, error=FALSE}
...
best_savr <- best_test(yb_mack/100, xb_mack, mack_b_1, weibbull_xb_mack, 
                       weibex_xb_mack, weibex2_xb_mack, expweib_xb_mack, 
                       stuexp3_xb_mack, stuexp2_xb_mack, exex_xb_mack, 
                       best_mack_b_1, best_weibbull_xb_mack, 
                       best_weibex_xb_mack, best_weibex2_xb_mack, 
                       best_expweib_xb_mack, best_stuexp3_xb_mack, 
                       best_stuexp2_xb_mack, best_exex_xb_mack, "SAVR")

best_tavr <- best_test(yr_mack/100, xr_mack, mack_r_1, weibbull_xr_mack, 
                       weibex_xr_mack, weibex2_xr_mack, expweib_xr_mack, 
                       stuexp3_xr_mack, stuexp2_xr_mack, exex_xr_mack, 
                       best_mack_r_1, best_weibbull_xr_mack, 
                       best_weibex_xr_mack, best_weibex2_xr_mack, 
                       best_expweib_xr_mack, best_stuexp3_xr_mack, 
                       best_stuexp2_xr_mack, best_exex_xr_mack, "TAVR")

tab <- matrix(c("PARTNER3", "LoRi", "TSR", best_tavr[1], best_tavr[2], 
                best_tavr[3], NA, NA, weibex_xr_mack[1:2], NA, 
                weibex_xr_mack[3:4], 
                "PARTNER3", "LoRi", "TSR",  best_savr[1], best_savr[2], 
                best_savr[3], NA, NA, weibex_xb_mack[1:2], NA, 
                weibex_xb_mack[3:4]), 
              ncol=13, byrow=TRUE)
...
# Speichern 
write.table(results, "results_mack.txt", sep = "\t", row.names = FALSE)
