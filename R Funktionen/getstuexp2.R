...
# Stückweise Exponentialfunktion mit 2 Parametern

getstuexp2 <- function (p = c(0.025, 0.5, 0.975), q, show.output = TRUE, plot = TRUE, 
                        start,
                                                                                 ...
                                                                                 0.9), wert1, ...) 
{
  ...
  minimize <- function(theta) {
    summand <- suppressWarnings(((q > 0 & q <= wert1 ) * (1 - exp(-theta[1] * q)) + 
                                   (q > wert1) *  (1 - exp(-theta[1] * wert1))  + 
                                   (exp(-wert1 * theta[2]) - exp(-theta[2] * q)))  - p)
  ...
  if (prod(!is.na(Par)) & plot) {
    ...
    main2 <- paste("2.para = ", signif(Par["2.para"], 
                                      digits = 3)) 
    main <- paste("2 stueckw. Exponential (", main1, ", ", main2, ")", sep = "")
    ...
    Probability <- (((Support > 0 & Support <= wert1 ) * (1 - exp(-Par["1.para"] * Support)) + 
                       (Support > wert1) * (1 - exp(-Par["1.para"] * wert1)) +
