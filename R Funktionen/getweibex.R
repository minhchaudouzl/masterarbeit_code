...
getweibex <- function (p = c(0.025, 0.5, 0.975), q, show.output = TRUE, plot = TRUE, 
                           start,
                                                                                    ...
                                                                                    0.9), ...) 
{
  ...
  minimize <- function(theta) {
    summand <- suppressWarnings((exp(theta[4]) / ( 1 + exp(theta[4])) * stats::pweibull(q = q,
                                                                shape = theta[1], scale = theta[2]) +
                                       (1 - exp(theta[4]) / ( 1 + exp(theta[4]))) * stats::pexp(q = q,
                                                                    rate = theta[3])) - p)
    #exp(theta[4]) / ( 1 + exp(theta[4]))
    #reliaR::pexpo.weibull(q = q, 
    #                      alpha = theta[1],
    #                      theta = theta[2]))
    #pexp einfügen aus stats
      # F(x) = x * ( mix * pweibull(x, rate, shape) + (1 - mix) * pexpo(x, scale) )
      #stats::pweibull(q = q, shape = theta[1], 
      #                                          scale = theta[2]) - p)
  ...
  if (is.error(try1) || fit$value >= tol) {
    ...
    if (is.error(try2) || fit$value >= tol) {
      ...
      Par <- fit$par
      names(Par) <- c("shape", "scale", "rate", "mix")
      if (show.output) 
        print(fit)
    }
  }
