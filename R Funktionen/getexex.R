...
# Mischung aus zwei Exponentialverteilungen

getexex <- function (p = c(0.025, 0.5, 0.975), q, show.output = TRUE, plot = TRUE, 
                       start,
                                                                                ...
                                                                                0.9), ...) 
{
  ...
  minimize <- function(theta) {
    summand <- suppressWarnings((exp(theta[3]) / ( 1 + exp(theta[3])) * stats::pexp(q = q,
                                                                                    rate = theta[1]) +
                                   (1 - exp(theta[3]) / ( 1 + exp(theta[3]))) * stats::pexp(q = q,
                                                                                            rate = theta[2])) - p)
  ...
  if (prod(!is.na(Par)) & plot) {
    main1 <- paste("rate1 = ", signif(Par["rate1"], 
                                     digits = 3))
    main2 <- paste("rate1 = ", signif(Par["rate2"], 
                                     digits = 3)) 
    main3 <- paste("mix = ", signif(exp(Par["mix"]) / ( 1 + exp(Par["mix"])), 
                                   digits = 3))
    main <- paste("Exponential & Exponential (", main1, ", ", main2, ", ", main3, ")", sep = "")
    sub = paste("fit.weights = c(", paste(fit.weights.original, 
                                          collapse = ", "), ")", sep = "")
    Support.lim <- c(0, 60)
    Support <- seq(Support.lim[1], Support.lim[2], length = 200)
    Probability <- (exp(Par["mix"]) / ( 1 + exp(Par["mix"])) *  stats::pexp(q = Support,
                                                                            rate = Par["rate1"]) +
