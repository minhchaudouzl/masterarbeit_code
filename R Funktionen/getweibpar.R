...
# exponentiierte Weibullverteilung mit 2 Parametern
# arbeitet mit reliaR::pexpo.weibull

getweibpar <- function (p = c(0.025, 0.5, 0.975), q, show.output = TRUE, plot = TRUE, 
                           start,
                                                                                    ...
                                                                                    0.9), ...) 
{
  ...
  minimize <- function(theta) {
    summand <- suppressWarnings((reliaR::pexpo.weibull(q = q,
                                                      alpha = theta[1],
                                                      theta = theta[2])) - p)
  ...
  if (prod(!is.na(Par)) & plot) {
    ...
    main2 <- paste("theta = ", signif(Par["theta"], 
                                     digits = 3))
    main <- paste("exp. Weibull (", main1, ", ", main2, 
                  ")", sep = "")
    sub = paste("fit.weights = c(", paste(fit.weights.original, 
                                          collapse = ", "), ")", sep = "")
    Support.lim <- c(0.001,60)
    Support <- seq(Support.lim[1], Support.lim[2], length = 200)
    Probability <- reliaR::pexpo.weibull(Support, Par["alpha"], 
                                         Par["theta"])
